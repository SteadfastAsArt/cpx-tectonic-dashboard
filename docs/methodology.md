...
## 9. Stratified Subset Evaluation via OOF Slicing

本项目还用到了一种在实验对比中常被忽略的评估范式——**stratified subset evaluation**，简称"OOF 切片"。它回答的问题是：一个在全量训练集上训练好的模型，**仅限评估其 OOF 预测在某个元数据定义的子集上的表现**，这个性能和"专门在该子集上重新训练一个模型"相比如何？

### 形式化

设训练集为 $\mathcal{D}=\{(x_i,y_i)\}_{i=1}^{N}$，通过 $K$ 折分层 CV 得到 OOF 预测向量 $\hat{y}^{\text{OOF}}=(\hat{y}_1,\ldots,\hat{y}_N)$，其中 $\hat{y}_i$ 是样本 $i$ 所在 fold 的模型给出的预测。现在给定一个由元数据定义的子集 $\mathcal{S}\subseteq\mathcal{D}$（例如 $\mathcal{S}=\{i:\text{ROCK NAME}_i\ni\text{BASALT}\land\text{CRYSTAL}_i\in\{\text{pheno},\text{microphenocryst}\}\}$），我们在 $\mathcal{S}$ 上计算指标：

$$F_1^{\text{macro}}(\mathcal{S}) = \frac{1}{|\mathcal{K}_{\mathcal{S}}|}\sum_{k\in\mathcal{K}_{\mathcal{S}}} F_1^{(k)}\!\left(\{y_i\}_{i\in\mathcal{S}},\{\hat{y}_i\}_{i\in\mathcal{S}}\right),$$

其中 $\mathcal{K}_{\mathcal{S}}$ 是子集中实际出现的类别集合（缺席的类不参与平均，以保证 apples-to-apples 地对比"只在这个 regime 上训练"的专用模型）。

### 与独立训练的对比

令 $\mathcal{M}_{\text{full}}$ 为在 $\mathcal{D}$ 上训练、在 $\mathcal{S}$ 上评估的主模型性能，$\mathcal{M}_{\text{sub}}$ 为仅在 $\mathcal{S}$ 上重新做 $K$ 折 CV 训练的专用模型性能。两者的差 $\Delta = \mathcal{M}_{\text{sub}} - \mathcal{M}_{\text{full}}$ 可以被分解成三种定性情况：

1. $\Delta \approx 0$：主模型已经在 $\mathcal{S}$ 上 generalize 充分，不需要专用模型。
2. $\Delta > 0$ 且 gap 主要来自大样本类：$\mathcal{S}$ 内部存在一个主模型未充分建模的 regime-specific 信号结构，值得训练专用模型或给主模型添加 regime 指示特征。
3. $\Delta > 0$ 且 gap 主要来自 $n_k \ll 20$ 的小样本类：$\mathcal{M}_{\text{sub}}$ 在小类上过拟合，对 $\Delta$ 的贡献是虚假的，主模型仍然是正确选择。

对这三种情况的区分**不能**通过全局 F1-macro 的单一数字完成，必须拆到每个类的 $F_1^{(k)}$ 上，观察差异主要落在哪一档样本量。

### 与其他常见方法的区别

Stratified subset evaluation 不同于几个容易混淆的概念：

- **它不是** out-of-distribution (OOD) 评估：$\mathcal{S}\subseteq\mathcal{D}$，子集样本在训练时被模型见过（虽然每个样本恰好被自己所在 fold 的模型从未见过，但 $\mathcal{S}$ 的联合分布已经落在训练分布内）。
- **它不是** holdout 验证：holdout 要求评估集从未在任何 fold 被训练，而 OOF 切片使用的是 CV 的自然 holdout 性质。
- **它不是** nested CV：nested CV 是超参搜索的嵌套，stratified subset evaluation 是对同一模型在不同 subset 上的重评估。

它最接近的类比是 **conditional risk analysis**——把模型的整体期望风险 $R = \mathbb{E}_{(x,y)}[\ell(f(x),y)]$ 分解为 $R = \sum_g \Pr(\mathcal{S}_g) \cdot \mathbb{E}_{(x,y)\in\mathcal{S}_g}[\ell(f(x),y)]$，其中 $\mathcal{S}_g$ 是任意元数据划分。只不过在 ML 实务里这种分解通常只做到"按类别"，很少按更细的元数据（ROCK NAME × CRYSTAL 这种岩石学维度）做。

### 应用：basalt+phenocryst 子集

本项目的一个具体应用：独立训练的"玄武岩斑晶 + excl-trace"模型给出 $F_1^{\text{macro}}=0.959$（$n=783$, 8 类），单看这个数字无法判断它到底是 regime 本身更易学，还是 $n$ 小导致过拟合。我们在 `eval_basalt_pheno_on_best_practical.py` 里复现了 `10_cv_confusion.py` 的完整 5 折 CV（seed、超参、类权全部一致，sanity check 再现了整体 $F_1^{\text{macro}}=0.9094$），把得到的 OOF 预测切到同一 783 个样本上，计算 $F_1^{\text{macro}}(\mathcal{S})=0.9027$。两个模型的 gap 是 $\Delta=0.056$，但按类拆开后发现：

- Continental Flood Basalt ($n=5$)：$0.667 \to 1.000$，贡献 $\Delta$ 的 42%；
- Oceanic Plateau ($n=16$)：$0.970 \to 1.000$，贡献约 7%；
- 6 个大样本类 ($n\ge 39$)：平均 $\Delta \approx 0.015$，其中 Seamount 和 Ocean Island 反而是主模型占优（$\Delta < 0$）。

按上文的分类这属于第 3 种情况：$\Delta$ 主要来自小样本类的过拟合，主模型的选择是正确的。同时这种切片还意外暴露了一个地质上可解释的事实——Intraplate Volcanics 在整体上 $F_1=0.956$，但在 basalt+phenocryst 切片上掉到 $0.848$，gap 10.8 个百分点，说明整体的 Intraplate 类主要由橄榄岩捕虏体驱动，真正的 intraplate basalt phenocryst 是其中的少数派。这是一个单靠整体指标或者单靠独立训练都看不到的数据结构性事实。

### 实施要点

若要在其他项目复用这个方法，有三个关键前提：

1. **相同的 CV 配置**：seed、折数、分层策略、超参、样本权重都必须和主模型完全一致，否则 sanity check 无法再现主模型的整体指标，后续切片结果也失去基线。
2. **$\mathcal{S}$ 不能参与训练集的任何部分再采样**：换言之，不能在切片定义里引入和 fold 分配相关的信息（例如用"仅使用 fold 1 的模型"去评估某个子集）。这会污染 OOF 的独立性。
3. **报告时必须同时给出主模型整体指标、切片指标、独立训练指标三个数字**，并拆到每个类上。只给一个 $\Delta$ 会让读者无法区分过拟合噪声和真实的 regime 信号。

本项目的 `eval_basalt_pheno_on_best_practical.py` 可作为该范式的参考实现。
